class Room1 extends Phaser.Scene {
    ...
    createInventoryUI() {
        ...
        this.inventoryBg = this.add.rectangle(x, y, 140, 140, 0x000000, 0.7)
            .setScrollFactor(0)
            .setDepth(2000);

        this.inventoryLabel = this.add.text(x, 20, 'INVENTORY', {
            fontSize: '16px',
            fill: '#ffffff',
            fontStyle: 'bold'
        })
        ...
        this.inventorySprite = this.add.sprite(x, y, 'interactables', 0)
            .setScale(3)
            .setScrollFactor(0)
            .setDepth(2001)
            .setVisible(false);

        this.inventoryEmptyText = this.add.text(x, y, 'EMPTY', {
            fontSize: '14px',
            fill: '#888888'
        })
            .setOrigin(0.5)
    ...
    createUI() {
        ...
        // Box counter (top-right, under inventory)
        this.boxCounterText = this.add.text(this.scale.width - 150, 160, '', {
            fontSize: '18px',
            fill: '#ffffff',
            backgroundColor: '#000000aa',
            padding: { x: 10, y: 6 }
    ...
    updateInteractions(delta) {
        ...
        const nonDomJustClosed = //closed fist w/ non dominant hand
            //this.nonDominantGrabbing && !this.prevNonDominantGrabbing && !this.bothHandsGrabbing;
            this.nonDominantGrabbing && !this.prevNonDominantGrabbing;

        const domJustOpened =
            this.dominantHandOpen && !this.prevDominantHandOpen;
        ...
        // --- STORE OR DROP (edge) ---
        //COME BACK TO THIS (DOMINANT HAND OPEN FIST AND HOLDING ITEM RESULTS IN DROPPING.)
        if (this.heldItemType && domJustOpened) {
