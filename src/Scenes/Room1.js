class Room1 extends Phaser.Scene {
    ...
    updateInteractions(delta) {
        ...
        if (this.nearbyInteractable) {
            console.log('Near:', this.nearbyInteractable.itemType,
                        'NonDom grabbing:', this.nonDominantGrabbing,
                        'Held:', this.heldItem !== null);
        }
        
        // CASE 1: Holding an item, dominant hand opens = DROP
        if (this.heldItem && this.dominantHandOpen) {
            this.dropItem();
            return;
        }
        
        // CASE 2: Near interactable
        if (this.nearbyInteractable && !this.heldItem) {
            // Non-dominant hand closed fist = PICK UP
        ...
        // CASE 3: Holding tape/knife near box
        if (this.heldItem && this.nearbyInteractable) {
            this.handleBoxInteraction(this.nearbyInteractable, deltaSeconds);
        }
    }
    ...
    pickupItem(item) {
        ...
        // Create held sprite (follows player in update())
        this.heldItemSprite = this.add.sprite(
            this.player.x,
            this.player.y - 24,
            'interactables',
            this.getFrameForType(item.itemType)
        );
